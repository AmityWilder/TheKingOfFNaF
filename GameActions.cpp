...
void ConfirmSystem(State system) {
    if (GAME_STATE.state != system) {
        if (GAME_STATE.state == State::Office) {
            ...
            SimulateMouseClickAt(GetButtonPos(Button(10 + (int)system)));
        }
    }
}
...
void OpenCameraIfClosed() {
    switch (GAME_STATE.state) {
        ...
        case State::Office:
            ...
            Sleep(CAM_RESP_MS);
            UpdateState();
            if (GAME_STATE.state != State::Camera) {
...
void EnterGameState(State state, Camera cam) {
    ...
    case State::Camera:
        OpenCameraIfClosed();
        if (GAME_STATE.stateData.cd.camera != cam) {
    ...
    case State::Duct:
        OpenMonitorIfClosed();
        UpdateState();
        if (GAME_STATE.state != State::Duct) {
    ...
    case State::Vent:
        OpenMonitorIfClosed();
        UpdateState();
        if (GAME_STATE.state != State::Vent) {
...
namespace action {
    ...
    void ResetVents() {
        ...
        GAME_STATE.gameData.ventilationNeedsReset = false;
        Sleep(10);
    }
...
void ActOnGameData() {
    ...
    constexpr int DECISECS_PER_HOUR = 450;

    if (GAME_STATE.state == State::Office) {
        if (CheckOnNMBB()) action::HandleNMBB();
    }

    if (GAME_STATE.gameData.ventilationNeedsReset) {
        action::ResetVents();
    }
